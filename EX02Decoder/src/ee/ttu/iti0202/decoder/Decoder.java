...
public class Decoder {
    ...
    private static String decodeMessage(String dictionary, String message) {
        ...
        for (int i = 0; i < message.length() - 1; i++) {
            ...
            switch (current) {                              // "1+2+354+1+2" => 123
                case LONG_INDEX:    int regexInt = regexer("^\\d[+]\\d([+]\\d)*", message.substring(i));
                                    result.append(caseDecider(upperCase, dictionary.charAt(regexInt)));
                ...
                case SHORT_INDEX:   int dictIndex = Integer.parseInt(String.valueOf(message.charAt(i)));
                    ...
                    break;
                default:    if (upperCase) {
                                result.append(Character.toUpperCase(message.charAt(i)));
                            } else {
